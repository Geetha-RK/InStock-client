  ...
  return (
    <MainCard className={block}>
      <div className={`${block}__head`}>
        <PageHeader hasBackButton={true}>{values.warehouse_name}</PageHeader>
        <Link
          to={`/warehouses/edit/${values.id}`}
          className={`${block}__edit-box`}
        >
          <img
            className={`${block}__edit-icon`}
            src={editicon}
            alt="edit item"
          />
      ...
      <Modal
        ...
        content={`Please confirm that you'd like to delete ${modalItem?.item_name} from the inventory list. You won't be able to undo this action.`}
